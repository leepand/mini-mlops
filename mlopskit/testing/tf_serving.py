...
from mlopskit.core.models.tensorflow_model import TensorflowModel, connect_tf_serving
from mlopskit.utils.tensorflow import deploy_tf_models
    ...
    if deployment == "process":  # pragma: no cover
        ...
        proc = subprocess.Popen(
            [
                ...
                "--model_config_file="
                f"{os.environ['MLOPSKIT_ASSETS_DIR']}/testing.config",
            ]
            + cmd
        )
        ...
        subprocess.call(
            ["docker", "rm", "-f", "mlopskit-tfserving-tests"],
        ...
        tfserving_proc = subprocess.Popen(
            [
                ...
                "run",
                "--name",
                "mlopskit-tfserving-tests",
                "--volume",
                f"{os.environ['MLOPSKIT_ASSETS_DIR']}:/config",
                "-p",
                "8500:8500",
                "-p",
                "8501:8501",
        ...
        def finalize():
            subprocess.call(["docker", "kill", "mlopskit-tfserving-tests"])
