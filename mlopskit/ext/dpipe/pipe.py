...
from .utils import filemd5, copytree
...
FILE_CFG = "~/mlopskit/cfg.json"
FILE_PATH = "~/mlopskit/"
if str(FILE_CFG).startswith("~"):
    filecfg_path = os.path.expanduser(FILE_CFG)
    with open(filecfg_path, "r") as f:
        filecfg_config = json.load(f)

DEFAULT_SETTINGS = {
    "protocol": "filebase",
    "role": "write",
    "options": {"remotepath": FILE_PATH},
}
...
class PipeBase(object):
    ...
    def __init__(self, name, version, sortby="filename"):
    ...
    def delete_files_remote(self, files=None, confirm=True):
        ...
        if c == "y":
            ...
            self._pullpush_model(files, op="remove")
...
class PipeLocal(PipeBase):
    ...
    def __init__(
        ...
        super().__init__(name, version, sortby)
        ...
        self.settings = (
            self.dbconfig.all()[-1]["pipe"] if self.dbconfig.all() else DEFAULT_SETTINGS
        )
...
class Pipe(PipeBase):
    ...
    def __init__(
        ...
        if self.settings["protocol"] not in ["s3", "ftp", "sftp", "filebase"]:
    ...
    def pull_preview(
        self, files=None, include=None, exclude=None, nrecent=0, cached=True
    ):
        """
    ...
    def pull(
        ...
        if files is not None:
            ...
            filespull = _files_diff(
                filesremote, fileslocal, self.mode, include, exclude, nrecent
            )
        ...
        fileslocalmod, _ = self.scan_local(
            files=[s for s in filespull if s in fileslocal], attributes=True
        )
        ...
        if filesmod:
            if merge_mode is None:
                raise PullError(
                    "Modified files will be overwritten by pull, specify merge mode "
                    + str(filesmod)
                )
    ...
    def push(
        ...
        filespush_size = sum(
            f["size"] for f in fileslocal if f["filename"] in filespush
        )
        if dryrun:
    ...
    def remove_orphans(self, files=None, direction="local", dryrun=None):
        ...
        filesrmremote = self._pullpush_model(filesrmremote, "remove")
    ...
    def _list_model(self):
        def scan_s3():
            ...
            filesall = [
                {
                    "filename": s3path(o)[idxStart:],
                    "modified_at": str(o.last_modified),
                    "size": o.size,
                    "crc": o.e_tag.strip('"'),
                }
                for o in filesall
        ...
        def scan_ftp():
            try:
                ...
                raise ModuleNotFoundError(
                    "pyftpsync not found. Run `pip install pyftpsync`"
                )
            ...
            try:
                ...
                filesall = [
                    {
                        "filename": str(PurePosixPath(o.rel_path) / o.name)[idxStart:],
                        "modified_at": datetime.fromtimestamp(o.mtime),
                        "size": o.size,
                        "crc": "{}-{}".format(str(o.mtime), str(o.size)),
                    }
                    for o in filesftp
                    if not o.is_dir()
                ]
        ...
        def scan_sftp():
            ...
            if cnxn.exists(self.remote_prefix):
                ...
                filesall = [
                    {
                        "filename": o.relpath,
                        "modified_at": datetime.fromtimestamp(o.st_mtime),
                        "size": o.st_size,
                        "crc": "{}-{}".format(str(o.st_mtime), str(o.st_size)),
                    }
                    for o in filesall
                ]
            else:
        ...
        if self.settings["protocol"] == "s3":
            ...
            filesall = scan_sftp()
        elif self.settings["protocol"] == "filebase":
            filesall = scan_filebase()
        else:
    ...
    def _pullpush_model(self, files, op, cnxn=None):
