...
class Experiment(object):
    ...
    def save(self):
        ...
        try:
            ...
            if is_new_record:
                ...
                pipe.hset(
                    self.key(), "created_at", datetime.now().strftime("%Y-%m-%d %H:%M")
                )
    ...
    def scan_and_delete(self, match=None, count=None):
        cursor = 0
        while True:
            result = self.redis.scan(cursor, match=match, count=count)
            cursor = result[0]
            keys = result[1]
            for key in keys:
                key_type = self.redis.type(key)
                if str(key_type) == "zset":
    ...
    def delete(self):
        ...
        match = "*:{0}:*".format(self.name)
        self.scan_and_delete(match=match, count=100)
    ...
    def traffic_fraction(self):
        if self._traffic_fraction is False:
            try:
                self._traffic_fraction = float(
                    self.redis.hget(self.key(), "traffic_fraction")
                )
    ...
    @classmethod
    def find_or_create(
        cls, experiment_name, alternatives, traffic_fraction=None, redis=None
    ):
        ...
        if sorted(experiment.get_alternative_names()) != sorted(alternatives):
            raise ValueError(
                "experiment alternatives have changed. please delete in the admin"
            )
    ...
    def archived(redis=None):
        experiments = Experiment.all(
            exclude_archived=False, exclude_paused=True, redis=redis
        )
    ...
    def paused(redis=None):
        experiments = Experiment.all(
            exclude_archived=True, exclude_paused=False, redis=redis
        )
    ...
    def is_valid(experiment_name):
        return (
            isinstance(experiment_name, basestring)
            and VALID_EXPERIMENT_ALTERNATIVE_RE.match(experiment_name) is not None
        )

    @staticmethod
    def validate_kpi(kpi):
        return isinstance(kpi, str) and VALID_KPI_RE.match(kpi) is not None
...
class Alternative(object):
    ...
    def __repr__(self):
        return "<Alternative {0} (Experiment {1})>".format(
            repr(self.name), repr(self.experiment.name)
        )
    ...
    def objectify_by_period(self, period, slim=False):
        ...
        dates = sorted(list(set(list(conversions.keys()) + list(participants.keys()))))
    ...
    def record_participation(self, client, dt=None):
        ...
        keys = [
            ...
            _key(
                "p:{0}:{1}:{2}".format(experiment_key, self.name, date.strftime("%Y"))
            ),
            _key(
                "p:{0}:{1}:{2}".format(
                    experiment_key, self.name, date.strftime("%Y-%m")
                )
            ),
            _key(
                "p:{0}:{1}:{2}".format(
                    experiment_key, self.name, date.strftime("%Y-%m-%d")
                )
            ),
        ]
        msetbit(
            keys=keys, args=([self.experiment.sequential_id(client), 1] * len(keys))
        )
    ...
    def record_conversion(self, client, dt=None):
        ...
        keys = [
            ...
            _key(
                "c:{0}:_all:users:{1}".format(experiment_key, date.strftime("%Y-%m-%d"))
            ),
            _key("c:{0}:{1}:users:all".format(experiment_key, self.name)),
            _key(
                "c:{0}:{1}:users:{2}".format(
                    experiment_key, self.name, date.strftime("%Y")
                )
            ),
            _key(
                "c:{0}:{1}:users:{2}".format(
    ...
    def g_stat(self):
        ...
        expected_control_conversions = (
            control.participant_count() * total_conversions / float(total_participants)
        )
        expected_alt_conversions = (
            self.participant_count() * total_conversions / float(total_participants)
        )
        expected_control_failures = (
            control.participant_count() - expected_control_conversions
        )
        expected_alt_failures = self.participant_count() - expected_alt_conversions

        try:
            g_stat = 2 * (
                alt_conversions * log(alt_conversions / expected_alt_conversions)
                + alt_failures * log(alt_failures / expected_alt_failures)
                + control_conversions
                * log(control_conversions / expected_control_conversions)
    ...
    def is_valid(alternative_name):
        return (
            isinstance(alternative_name, str)
            and VALID_EXPERIMENT_ALTERNATIVE_RE.match(alternative_name) is not None
        )
