...
class CORSMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        def add_cors_headers(status, headers, exc_info=None):
            ...
            headers.add("Access-Control-Allow-Origin", get_origin(status, headers))
            headers.add("Access-Control-Allow-Headers", cfg.get("cors_headers"))
            headers.add("Access-Control-Allow-Credentials", cfg.get("cors_credentials"))
            headers.add("Access-Control-Allow-Methods", cfg.get("cors_methods"))
            headers.add("Access-Control-Expose-Headers", cfg.get("cors_expose_headers"))
...
class Sixpack(object):
    ...
    def dispatch_request(self, request):
        ...
        try:
            ...
            return json_error(
                {"message": "an internal error has occurred"}, request, 500
            )
    ...
    def on_convert(self, request):
        ...
        try:
            alt = convert(
                experiment_name, client_id, kpi=kpi, datetime=dt, redis=self.redis
            )
