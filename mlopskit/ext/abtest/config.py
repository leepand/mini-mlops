...
if config_path is None:
    h_p = data_dir()
    config_path_home = os.path.join(h_p, "mlops_config.yml")
    conf = YAMLDataSet(config_path_home).load()
    REDIS_PORT = conf["redis_port"]
    REDIS_HOST = conf["redis_host"]
    REDIS_DB = conf["redis_ab_db"]
if config_path:
    try:
    ...
    CONFIG = {
        ...
        "redis_port": int(os.environ.get("SIXPACK_CONFIG_REDIS_PORT", REDIS_PORT)),
        "redis_host": os.environ.get("SIXPACK_CONFIG_REDIS_HOST", REDIS_HOST),
        ...
        "redis_socket_timeout": os.environ.get(
            "SIXPACK_CONFIG_REDIS_SOCKET_TIMEOUT", None
        ),
        "redis_sentinel_service_name": os.environ.get(
            "SIXPACK_CONFIG_REDIS_SENTINEL_SERVICE_NAME", None
        ),
        "redis_max_connections": int(
            os.environ.get("SIXPACK_CONFIG_REDIS_MAX_CONNECTIONS", "0")
        ),
        "redis_db": int(os.environ.get("SIXPACK_CONFIG_REDIS_DB", REDIS_DB)),
        "robot_regex": os.environ.get(
            "SIXPACK_CONFIG_ROBOT_REGEX",
            "$^|trivial|facebook|MetaURI|butterfly|google|"
