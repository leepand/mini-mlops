...
class Ls:
        ...
        *,
        ...
        if include:
            ...
            include_filters.extend(
                _make_ls_filter(
                    only_files=only_files, only_dirs=only_dirs, filterable=incl
                )
            ...
            include_filters.append(
                _make_ls_filter(only_files=only_files, only_dirs=only_dirs)
            )
        ...
        if exclude:
            ...
            exclude_filters.extend(
                _make_ls_filter(filterable=excl) for excl in excludes
            )
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(path={self.path!r}, recursive={self.recursive})"
        )
    ...
    with scanner:
        while True:
            ...
            if (
                recursive
                and not excluded
                and entry_path.is_dir()
                and not entry_path.is_symlink()
            ):
...
def _make_ls_filter(
    only_files: bool = False,
    only_dirs: bool = False,
    filterable: t.Optional[LsFilterable] = None,
) -> LsFilterFn:
