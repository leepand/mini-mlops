...
@contextmanager
def atomicdir(
    dir: StrPath, *, skip_sync: bool = False, overwrite: bool = True
) -> t.Iterator[Path]:
    """
    ...
    if dst.is_file():
        raise FileExistsError(
            errno.EEXIST, f"Atomic directory target must not be a file: {dst}"
        )
    ...
    **open_kwargs: t.Any,
    ...
    if dst.is_dir():
        raise IsADirectoryError(
            errno.EISDIR, f"Atomic file target must not be a directory: {dst}"
        )
