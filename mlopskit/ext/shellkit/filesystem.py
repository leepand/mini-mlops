...
CHMOD_SYMBOLIC_PATTERN = re.compile(
    r"^(?P<who>[ugoa]*)(?P<op>[+\-=])(?P<perm>[ugo]|[rwxst]*)$"
)
    ...
    *,
    ...
    if recursive and isinstance(path, Path) and path.is_dir():
        for subpath in walk(path):
            ...
            chmod(
                subpath, original_mode, follow_symlinks=follow_symlinks, recursive=False
            )
    ...
    for item in items:
        ...
        for who_char, perm_char in itertools.product(who, perm):
            if perm_char in "ugo":
                ...
                submask = _get_inherited_symbolic_mode(
                    mode, to_who=who_char, from_who=perm_char
                )
            else:
                symbol = who_char + perm_char
                if symbol not in CHMOD_SYMBOLIC_TABLE:
                    raise ValueError(
                        f"chmod: Unsupported symbolic mode: {symbolic_mode}"
                    )
    ...
    if src.is_dir():
        if dst.exists() and not dst.is_dir():
            raise FileExistsError(
                errno.EEXIST,
                f"Cannot copy {src!r} to {dst!r} since destination is a file",
            )
        ...
        if dst.is_dir():
            ...
            shutil.copytree(
                src, dst, symlinks=not follow_symlinks, copy_function=copy_function
            )
    else:
        if dst.is_dir():
            dst = dst / src.name
