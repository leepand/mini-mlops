...
class BaseArchive(ABC):
    ...
    def addsource(
        self, source: ArchiveSource, arcname: t.Optional[StrPath] = None
    ) -> None:
...
def archive(
    ...
    *paths: t.Union[StrPath, Ls],
    ...
    _verify_archive_root(
        root, [source for source in sources if str(source) not in repath]
    )
    ...
    with atomicfile(file, "wb", skip_sync=True) as fp:
        with archive_class.open(fp, "w") as archive_file:
            try:
                ...
                raise ArchiveError(
                    f"archive: Failed to create archive '{file}' due to error: {exc}",
                    orig_exc=exc,
                ) from exc
    ...
    *,
    ...
    if src == dst:
        raise FileExistsError(
            errno.EEXIST, f"Backup destination cannot be the source: {src}"
        )
...
def unarchive(
    file: StrPath, dst: StrPath = ".", *, ext: str = "", trusted: bool = False
) -> None:
    """
