...
class Command:
    ...
    def shell_cmd(self) -> str:
        ...
        cmd = " ".join(
            shlex.quote(arg.decode() if isinstance(arg, bytes) else arg)
    ...
    @classmethod
    def from_command(
        cls, command, *extra_args: RunArgs, **override_kwargs: t.Any
    ) -> "Command":
    ...
    def run(
        self, *extra_args: RunArgs, **override_kwargs: t.Any
...
def _parse_run_args(
    args: tuple, error_prefix: str = "run(): "
) -> t.List[t.Union[str, bytes]]:
    good_args = []
    bad_args = []
    ...
    if not good_args:
        raise TypeError(
            f"{error_prefix}requires at least one non-empty positional argument"
        )
