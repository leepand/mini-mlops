...
user_tag = {"author": default_user_name}
...
class MLflowRESTClient:
    ...
    def list_experiments(
        self, view_type: RunViewType = RunViewType.ACTIVE
    ) -> list[Experiment]:
        """
        ...
        data = parse_obj_as(
            List[Experiment],
            self._get("experiments/list", view_type=view_type.value).get(
                "experiments", []
            ),
        )
        return data

    def list_experiments_iterator(
        self, view_type: RunViewType = RunViewType.ACTIVE
    ) -> Iterator[Experiment]:
        """
    ...
    def get_experiment(self, experiment_id: int) -> Experiment:
        ...
        data = self._get("experiments/get", experiment_id=experiment_id).get(
            "experiment"
        )
    ...
    def create_experiment(
        ...
        experiment_id = self._post("experiments/create", name=name, **params).get(
            "experiment_id"
        )
        default_user_name
        uid = "author"
        name = default_user_name
        self.set_experiment_tag(experiment_id, key=uid, value=name)
    ...
    def get_run(self, run_id: RunId) -> Run:
        ...
        return Run.parse_obj(
            self._get("runs/get", run_id=UUID(str(run_id)).hex).get("run")
        )
    ...
    def create_run(
        ...
        if not tags:
            ...
            if isinstance(tags, dict):
                tags.update(user_tag)
            else:
                tags.append({"key": "author", "value": default_user_name})
    ...
    def set_run_status(
        ...
        return RunInfo.parse_obj(
            self._post("runs/update", run_id=UUID(str(run_id)).hex, **params).get(
                "run_info"
            )
        )
    ...
    def finish_run(
        self, run_id: RunId, end_time: int | datetime | None = None
    ) -> RunInfo:
        """
        ...
        return self.set_run_status(
            run_id, RunStatus.FINISHED, end_time=format_to_timestamp(end_time)
        )
    ...
    def kill_run(
        ...
        data = self.set_run_status(
            run_id, RunStatus.KILLED, end_time=format_to_timestamp(end_time)
        )
    ...
    def log_run_parameter(self, run_id: RunId, key: str, value: str) -> None:
        ...
        self._post(
            "runs/log-parameter", run_id=UUID(str(run_id)).hex, key=key, value=value
        )
    ...
    def log_run_metrics(
        ...
        metrics_list = [
            self._add_timestamp(metric, format_to_timestamp(timestamp))
            for metric in self._handle_tags(metrics)
        ]
    ...
    def list_run_metric_history(self, run_id: RunId, key: str) -> list[Metric]:
        ...
        return parse_obj_as(
            List[Metric],
            self._get(
                "metrics/get-history", run_id=UUID(str(run_id)).hex, metric_key=key
            ).get("metrics"),
        )

    def list_run_metric_history_iterator(
        self, run_id: RunId, key: str
    ) -> Iterator[Metric]:
        """
    ...
    def list_run_artifacts(
        self, run_id: RunId, path: str | None = None, page_token: str | None = None
    ) -> Page:
        """
        List run artifacts
        Parameters
        ...
        return Page.make(
            response, items_key="files", item_class=Artifact, root=response["root_uri"]
        )
    ...
    def list_run_artifacts_iterator(
        ...
        while True:
            ...
            if page.has_next_page:
                page = self.list_run_artifacts(
                    run_id=run_id, path=path, page_token=page.next_page_token
                )
            else:
                break
    ...
    def create_model(self, name: str, tags: TagsListOrDict | None = None) -> Model:
        ...
        tags_list = self._handle_tags(tags)
        data = self._post("registered-models/create", name=name, tags=tags_list).get(
            "registered_model"
        )
    ...
    def get_model(self, name: str) -> Model:
        ...
        return Model.parse_obj(
            self._get("registered-models/get", name=name).get("registered_model")
        )

    def get_or_create_model(
        self, name: str, tags: TagsListOrDict | None = None
    ) -> Model:
        """
    ...
    def rename_model(self, name: str, new_name: str) -> Model:
        ...
        return Model.parse_obj(
            self._post("registered-models/rename", name=name, new_name=new_name).get(
                "registered_model"
            )
        )
    ...
    def set_model_description(self, name: str, description: str) -> Model:
        ...
        return Model.parse_obj(
            self._patch(
                "registered-models/update", name=name, description=description
    ...
    def list_models(
        self, max_results: int = MAX_RESULTS, page_token: str | None = None
    ) -> Page:
        """
        List models
        Parameters
    ...
    def list_model_versions(
        ...
        response = self._get(
            "registered-models/get-latest-versions", name=name, **params
        )
    ...
    def list_model_all_versions(
        ...
        return parse_obj_as(
            ListableModelVersion,
            self.list_model_all_versions_iterator(name=name, stages=stages),
        )
    ...
    def create_model_version(
        ...
        return ModelVersion.parse_obj(
            self._post(
                "model-versions/create", name=name, tags=tags_list, **params
            ).get("model_version"),
        )
    ...
    def get_model_version(self, name: str, version: int) -> ModelVersion:
        ...
        return ModelVersion.parse_obj(
            self._get("model-versions/get", name=name, version=str(version)).get(
                "model_version"
            )
        )

    def set_model_version_description(
    ...
    def delete_model_version_tag(self, name: str, version: int, key: str) -> None:
        ...
        self._delete(
            "model-versions/delete-tag", name=name, version=str(version), key=key
        )
    ...
    def get_model_version_download_url(self, name: str, version: int) -> str | None:
        ...
        return self._get(
            "model-versions/get-download-uri", name=name, version=str(version)
        ).get("artifact_uri")
    ...
    def test_model_version(self, name: str, version: int, **params) -> ModelVersion:
        ...
        return self.transition_model_version_stage(
            name, version, stage=ModelVersionStage.TEST, **params
        )
    ...
    def promote_model_version(self, name: str, version: int, **params) -> ModelVersion:
        ...
        return self.transition_model_version_stage(
            name, version, stage=ModelVersionStage.PROD, **params
        )
    ...
    def archive_model_version(self, name: str, version: int, **params) -> ModelVersion:
        ...
        return self.transition_model_version_stage(
            name, version, stage=ModelVersionStage.ARCHIVED, **params
        )
    ...
    def _request(
        self, method: str, url: str, log_response: bool = True, **params
    ) -> requests.Response:
        url = self._url(url)
