...
ModelVersionStageOrList = Union[
    str, ModelVersionStage, List[ModelVersionStage], List[str]
]
...
class ModelVersionStatus(BaseModel):
    ...
    def valid_state(cls, val):  # pylint: disable=no-self-argument
        return (
            ModelVersionState(val)
            if isinstance(val, str)
            else ModelVersionState.PENDING
        )

    def __str__(self):
        return str(self.state.name) + (
            f" because of '{self.message}'" if self.message else ""
        )
...
class Model(BaseModel):
    ...
    versions: ListableModelVersion = Field(
        default_factory=list, alias="latest_versions"
    )
    created_time: datetime = Field(None, alias="creation_timestamp")
    updated_time: datetime = Field(None, alias="last_updated_timestamp")
    description: str = ""
