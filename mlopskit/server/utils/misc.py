...
def is_valid_subpath2(relative_directory, base_directory):
...
def cat_file_content(file, fname):
    if fname.endswith(".py"):
        ...
        code_content = "该格式暂不支持"

    return code_content


def process_files(
    directory_files, base_directory, exclude=[], p_type="list", catfilename=None
):
    files = []
    for file in directory_files:
        ...
        PROCESS_TYPES = ("list", "cat")
        assert (
            p_type in PROCESS_TYPES
        ), f"process files/dir type {p_type} not supported. Valid types are {PROCESS_TYPES}"
        if p_type == "cat":
            fname = file.name
            print(fname, catfilename)
            if fname != catfilename:
                continue
            if fname.endswith(".py"):
                with open(file, "rb") as f:
                    source = f.read()
                ...
                formatter = formatters.HtmlFormatter(
                    linenos=True, full=True, style=style
                )
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if num_fails <= max_fails:
                ...
                print(
                    ("The rest of the `{}` errors " + "are hidden.").format(func_name)
                )
...
def wait_until_port_used(port, max_wait_sec=20, interval_sec=0.5):
    """
